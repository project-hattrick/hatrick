...
import { SIGNATURE_FILTERS, ScreenFilterDefs, ScreenFilterLayers, canvasFilterCss } from './screen-filters';

const HOME = AWAY_TEAM_PACKS.argentina;
const AWAY = AWAY_TEAM_PACKS.norway;

/**
 * Argentina (home) vs Norway (away) — both recolored packs via personaBodyRoot + personaBodyRootAway.
 * Runs every feel technique on + the baked "Signature" screen filter (no live filter editor).
 */
export function ArgentinaVsNorwayArena() {
  ...
  // France-vs-NL geometry, but both bodies swapped to recolored packs, and every feel technique on.
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 select-none overflow-hidden bg-[#06222f]">
      <canvas
        ...
        style={{ imageRendering: 'pixelated', filter: canvasFilterCss(SIGNATURE_FILTERS), transition: 'filter 0.25s ease' }}
      />
