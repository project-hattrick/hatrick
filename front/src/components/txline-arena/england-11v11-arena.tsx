...
import { ENGLAND_COMPLETE_SIZE_VARIANTS, type FranceSizeVariantId } from '@/game/realgk/config';
...
const isVariantId = (v: string | null): v is FranceSizeVariantId => v !== null && v in ENGLAND_COMPLETE_SIZE_VARIANTS;

/**
 * Full 11-a-side England match (England body pack keyed + trimmed from the AI review frames) with EVERY
 * smoothing technique on (feel = all) and the exported "Signature" screen-filter look applied by default
 * (live-tunable). Mirrors the France 11v11 arena; plays on the shared France stadium court.
 */
export function England11v11Arena() {
  ...
  const config = useMemo(() => ({ ...ENGLAND_COMPLETE_SIZE_VARIANTS[variant].config, feel: allFeelOn() }), [variant]);
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 select-none overflow-hidden bg-[#06222f]">
      ...
      <GoalBurst
        active={goalActive}
        team={goalTeam}
        blueName="England Blue"
        redName="England Red"
        scoreBlue={scoreBlue}
        scoreRed={scoreRed}
        clock={clock}
        accent={goalTeam === 'red' ? '#CE1124' : '#1d4ed8'}
      />
      <RestartBanner active={restartActive} label={restartLabel} team={restartTeam} teamName={restartTeam === 'red' ? 'England Red' : 'England Blue'} />
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 flex flex-col items-center gap-2 p-3">
        <div className="pointer-events-auto flex items-center gap-4 rounded-full border border-white/10 bg-black/55 px-5 py-2 backdrop-blur">
          <span className="max-w-[24vw] truncate text-right text-sm font-semibold text-[#7aa2ff]">England Blue</span>
          <span className="font-mono text-2xl font-bold tabular-nums text-white">
            {scoreBlue}<span className="mx-1 text-white/40">-</span>{scoreRed}
          </span>
          <span className="max-w-[24vw] truncate text-left text-sm font-semibold text-[#ff6b6b]">England Red</span>
        ...
        <Badge variant="outline" className="pointer-events-auto border-[#5f8fff]/40 bg-black/45 text-[#9db9ff]">
          England · 11v11 · all feel effects + signature filter
        </Badge>
      </div>
      ...
      <div className="absolute right-3 top-3 flex flex-wrap items-center justify-end gap-2">
        <div className="flex items-center overflow-hidden rounded-md border border-white/15 bg-black/45">
          {(Object.keys(ENGLAND_COMPLETE_SIZE_VARIANTS) as FranceSizeVariantId[]).map((id) => (
            <button
              key={id}
              type="button"
            ...
            >
              {ENGLAND_COMPLETE_SIZE_VARIANTS[id].label}
            </button>
          ))}
        </div>
