  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between gap-2 text-xs">
        <span className="w-8 shrink-0 text-left font-bold tabular-nums text-foreground">{home}</span>
        <span className="min-w-0 truncate text-center text-xs font-semibold text-muted-foreground">{label}</span>
        <span className="w-8 shrink-0 text-right font-bold tabular-nums text-foreground">{away}</span>
      </div>
